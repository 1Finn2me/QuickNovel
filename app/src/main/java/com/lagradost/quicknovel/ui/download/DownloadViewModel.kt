...
class DownloadViewModel : ViewModel() {
    ...
    val context: Context? get() = com.lagradost.quicknovel.BaseApplication.context
    ...
    fun readEpub(card: DownloadFragment.DownloadDataLoaded) = ioSafe {
        try {
            ...
            setKey(DOWNLOAD_EPUB_LAST_ACCESS, card.id.toString(), currentTime)
            cardsDataMutex.withLock {
                cardsData[card.id] = cardsData[card.id]?.copy(
                    generating = false,
                    lastReadTimestamp = currentTime  // Add this to trigger recomposition
                ) ?: return@withLock
            }
            postCards()
        }
